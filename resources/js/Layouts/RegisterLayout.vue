<script setup>
import getRegions from '../Services/GetRegions.js';
...
onMounted(async () => { listProvinsi.value = (await getRegions.getProvinsi())?.data; });
...
watch(() => formData.provinsi, async (val) => {
    ...
    listKabupaten.value = (await getRegions.getKabupaten(val.id))?.data;
  }
);
...
watch(() => formData.kabupaten, async (val) => {
    ...
    listKecamatan.value = (await getRegions.getKecamatan(val.id))?.data;
  }
);
...
watch(() => formData.kecamatan, async (val) => {
    ...
    listKelurahan.value = (await getRegions.getKelurahan(val.id))?.data;
  }
);
